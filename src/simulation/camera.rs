...
pub fn setup_follow_camera(mut commands: Commands, flyer_query: Query<&Transform, With<Flyer>>) {
    if let Ok(flyer_transform) = flyer_query.get_single() {
        let camera_position = flyer_transform.translation + Vec3::new(-15.0, 8.0, 0.0);
        
        commands.spawn((
            Camera3d::default(),
            Transform::from_translation(camera_position)
                .looking_at(flyer_transform.translation, Vec3::Y),
...
pub fn update_follow_camera(
    ...
    if let (Ok((flyer_transform, flyer_dynamics)), Ok((mut camera_transform, mut follow_camera))) = 
        (flyer_query.get_single(), camera_query.get_single_mut()) {
